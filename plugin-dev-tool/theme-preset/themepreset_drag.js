...
function updatePresetList() {
    ...
    listContainer.innerHTML = presets.map((preset, index) => {
        const date = new Date(preset.timestamp).toLocaleDateString();
        return `
        <div class="preset-item" draggable="true" data-index="${index}" data-name="${escapeHtml(preset.name)}" style="
            display: flex;
            align-items: center;
            gap: 10px;
            padding: 12px 14px;
    ...
    listContainer.querySelectorAll('.preset-item').forEach(item => {
        ...
        item.addEventListener('drop', (e) => {
            ...
            const [removed] = allPresets.splice(draggedIndex, 1);
            allPresets.splice(newIndex, 0, removed);
